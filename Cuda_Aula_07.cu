...
int main(int argc, char** argv) {
    ...
    { // Use a scope block to manage local variables for the histogram
        ...
        int histSize = 256;
        int hist_w = 600, hist_h = 500; // Increased size for labels
        int margin = 50;
        int graph_w = hist_w - 2 * margin;
        int graph_h = hist_h - 2 * margin;
        int bin_w = cvRound((double)graph_w / histSize);
        ...
        for (int i = 0; i < histSize; i++) {
            int red_h = cvRound(((double)redHistogram[i] / maxRed) * graph_h);
            int green_h = cvRound(((double)greenHistogram[i] / maxGreen) * graph_h);
            int blue_h = cvRound(((double)blueHistogram[i] / maxBlue) * graph_h);

            cv::rectangle(redHistImage, cv::Point(margin + bin_w * i, hist_h - margin - red_h), cv::Point(margin + bin_w * (i + 1), hist_h - margin), cv::Scalar(0, 0, 255), -1);
            cv::rectangle(greenHistImage, cv::Point(margin + bin_w * i, hist_h - margin - green_h), cv::Point(margin + bin_w * (i + 1), hist_h - margin), cv::Scalar(0, 255, 0), -1);
            cv::rectangle(blueHistImage, cv::Point(margin + bin_w * i, hist_h - margin - blue_h), cv::Point(margin + bin_w * (i + 1), hist_h - margin), cv::Scalar(255, 0, 0), -1);
        }

        // --- Add plot labels and axes ---
        int fontFace = cv::FONT_HERSHEY_SIMPLEX;
        double fontScale = 0.4;
        int thickness = 1;
        cv::Scalar textColor = cv::Scalar(220, 220, 220);
        cv::Scalar axisColor = cv::Scalar(150, 150, 150);

        auto drawAxes = [&](cv::Mat& img, int maxVal) {
            // Draw Y axis line
            cv::line(img, cv::Point(margin, hist_h - margin), cv::Point(margin, margin), axisColor, 1);
            // Draw X axis line
            cv::line(img, cv::Point(margin, hist_h - margin), cv::Point(hist_w - margin, hist_h - margin), axisColor, 1);

            // Y-axis labels and ticks
            int num_y_ticks = 5;
            for (int i = 0; i <= num_y_ticks; ++i) {
                int y_pos = hist_h - margin - i * (graph_h / num_y_ticks);
                cv::line(img, cv::Point(margin - 5, y_pos), cv::Point(margin, y_pos), axisColor, 1);

                std::stringstream ss;
                ss << std::fixed << std::setprecision(0) << (double)i * maxVal / num_y_ticks;
                cv::putText(img, ss.str(), cv::Point(5, y_pos + 5), fontFace, fontScale, textColor, thickness);
            }

            // X-axis labels and ticks
            int num_x_ticks = 4;
            for (int i = 0; i <= num_x_ticks; ++i) {
                int x_pos = margin + i * (graph_w / num_x_ticks);
                cv::line(img, cv::Point(x_pos, hist_h - margin), cv::Point(x_pos, hist_h - margin + 5), axisColor, 1);
                cv::putText(img, std::to_string(i * (histSize / num_x_ticks)), cv::Point(x_pos - 10, hist_h - margin + 20), fontFace, fontScale, textColor, thickness);
            }

            // Axis titles
            cv::putText(img, "Frequency", cv::Point(15, margin - 10), fontFace, 0.5, textColor, thickness);
            cv::putText(img, "Pixel Intensity", cv::Point(hist_w / 2 - 50, hist_h - 15), fontFace, 0.5, textColor, thickness);
        };

        drawAxes(redHistImage, maxRed);
        drawAxes(greenHistImage, maxGreen);
        drawAxes(blueHistImage, maxBlue);
